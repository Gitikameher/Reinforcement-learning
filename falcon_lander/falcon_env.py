...
class FalconLander():
    ...
    def control_floating_platform(self):
        ...
        if not GOING_LEFT:
            self.floating_drone_ship.ApplyForce(force=(CONST_FORCE_DRONE_SHIP * fx, CONST_FORCE_DRONE_SHIP * new_y_p2),
                                                point=p2, wake=True)
            self.floating_drone_ship.ApplyForce(
                force=(CONST_FORCE_DRONE_SHIP / 2 * fx, CONST_FORCE_DRONE_SHIP / 2 * new_y_p1), point=p1, wake=True)
            if width > (VIEWPORT_W / SCALE * 0.7):
                GOING_LEFT = True
        else:
            self.floating_drone_ship.ApplyForce(
                force=((-1) * CONST_FORCE_DRONE_SHIP * fx, -CONST_FORCE_DRONE_SHIP * new_y_p1), point=p1, wake=True)
            self.floating_drone_ship.ApplyForce(
                force=((-1) * CONST_FORCE_DRONE_SHIP / 2 * fx, -CONST_FORCE_DRONE_SHIP / 2 * new_y_p2), point=p2,
                wake=True)
    ...
    def _step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (not self.continuous and action == 2):
            ...
            if self.continuous:
                ...
                m_power = 1.0
            orient_x = tip[0] * (4 / SCALE + 2 * dispersion[0]) + side[0] * dispersion[
            ...
            p.ApplyLinearImpulse((orient_x * MAIN_ENGINE_POWER * m_power, orient_y * MAIN_ENGINE_POWER * m_power),
                                 impulse_pos, True)
            self.falcon_rocket.ApplyLinearImpulse(
                (-orient_x * MAIN_ENGINE_POWER * m_power, -orient_y * MAIN_ENGINE_POWER * m_power), impulse_pos, True)
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (not self.continuous and action in [1, 3]):
            ...
            impulse_pos = (self.falcon_rocket.position[0] + orient_x - tip[0] * 17 / SCALE,
                           self.falcon_rocket.position[1] + orient_y + tip[1] * SIDE_ENGINE_HEIGHT / SCALE)
            p = self._create_particle(0.7, impulse_pos[0], impulse_pos[1], s_power)
            p.ApplyLinearImpulse((orient_x * SIDE_ENGINE_POWER * s_power, orient_y * SIDE_ENGINE_POWER * s_power),
                                 impulse_pos, True)
            self.falcon_rocket.ApplyLinearImpulse(
                (-orient_x * SIDE_ENGINE_POWER * s_power, -orient_y * SIDE_ENGINE_POWER * s_power), impulse_pos, True)
...
def heuristic(env, s):
    ...
    angle_targ = (s[0] - s[8]) * 0.5 + s[
        2] * 0.85  # angle should point towards center (s[0] is horizontal coordinate, s[2] horiz. speed)
    if angle_targ > 0.3:
