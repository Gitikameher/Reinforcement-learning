...
def turn_experiences_into_subcategories(experiences):
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed,temperature,type_of_update,type_of_loss):
...
class ReplayBuffer:
    ...
    def sample(self,sample_weights=None):
        ...
        indexes=np.random.choice(len(self.memory), size=self.batch_size, replace=False, p=sample_weights)
        experiences=[]
        for i in range(len(indexes)):
            experiences.append(self.memory[indexes[i]])
        states, actions, rewards, next_states, dones=turn_experiences_into_subcategories(experiences)
        return (states, actions, rewards, next_states, dones)    
